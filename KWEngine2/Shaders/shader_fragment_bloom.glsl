...
void main()
{
	...
	if(uHorizontal > 0)
    {
        ...
        for(int i = 1; i < 8; i++)
        {
            result += texture(uTextureBloom, vTexture + vec2(tex_offset.x * (float(i) * (uResolution.x * float(i))), 0.0)).rgb * (weight[i] * multiplier);
            result += texture(uTextureBloom, vTexture - vec2(tex_offset.x * (float(i) * (uResolution.x * float(i))), 0.0)).rgb * (weight[i] * multiplier);
        }
		color.x = result.x;
		color.y = result.y;
    ...
    else
    {
        ...
        for(int i = 1; i < 8; i++)
        {
            result += texture(uTextureBloom, vTexture + vec2(0.0, tex_offset.y * (float(i) * (uResolution.y * float(i))))).rgb * (weight[i] * multiplier);
            result += texture(uTextureBloom, vTexture - vec2(0.0, tex_offset.y * (float(i) * (uResolution.y * float(i))))).rgb * (weight[i] * multiplier);
        }	
